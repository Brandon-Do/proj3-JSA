...
@app.route("/success")
def success():
    return flask.jsonify(template=flask.render_template('success.html'))
#######################
# Form handler.
# CIS 322 note:
...
def check():
    ...
    if matched and in_jumble and not (text in matches):
        ...
        response = "You found {}!".format(text)
        return flask.jsonify(message=response, found=matches, target=flask.session["target_count"])

    elif text in matches:
        response = "You already found {}".format(text)
        if text.strip() == "":
            response = ""
        return flask.jsonify(message=response, found=matches, target=flask.session["target_count"])

    elif not matched:
        response = "{} isn't in the list of words".format(text)
        return flask.jsonify(message=response, found=matches, target=flask.session["target_count"])

    elif not in_jumble:
        response = '"{}" can\'t be made from the letters {}'.format(text, jumble)
        return flask.jsonify(message=response, found=matches, target=flask.session["target_count"])
